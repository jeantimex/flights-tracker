...
let scene, camera, renderer, controls, earth, flights, gui, instancedPlanes, mergedFlightPaths, stats, stars, ambientLight, directionalLight;
...
function init() {
    ...
    const allFlights = flightData.map((flightOptions, index) => {
        const flight = new Flight(flightOptions, earth, instancedPlanes, index, mergedFlightPaths);
        return flight;
    });
...
function setupGUI() {
    ...
    planeFolder.add(guiControls, 'planeSize', 0.1, 3.0, 0.1)
        .name('Size')
        .onChange((value) => {
    ...
    animationFolder.add(guiControls, 'animationSpeed', 0.1, 3.0, 0.1)
        .name('Speed')
        .onChange((value) => {
    ...
    flightFolder.add(guiControls, 'flightCount', 1, flightData.length, 1)
        .name('Count')
        .onChange((value) => {
    ...
    lightingFolder.add(guiControls, 'dayNightEffect')
        .name('Day/Night Effect')
        .onChange((value) => {
            toggleDayNightEffect(value);
        });

    lightingFolder.add(guiControls, 'atmosphereEffect')
