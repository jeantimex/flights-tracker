...
export class Flight {
    constructor(flightOptions, earth, instancedPlanes, instanceId, mergedFlightPaths) {
    ...
    createFlightPath() {
        ...
        const startPoint = this.origin.clone().normalize().multiplyScalar(this.earth.getRadius() + lowAltitude);
        const endPoint = this.destination.clone().normalize().multiplyScalar(this.earth.getRadius() + lowAltitude);

        // Create mid point at flight altitude
        const midPoint = startPoint.clone().lerp(endPoint, 0.5).normalize().multiplyScalar(this.earth.getRadius() + flightAltitude);
